...
- **两种管理方式**：
  - 直接对 bot 说：「给 123456789 加个别名 闲聊群」「把 闲聊群的别名 改成 摸鱼群」「把 闲聊群的别名 钓鱼群 删掉」「看看有哪些别名」；
...
| 消息预览 | 按目标 + 日期范围像聊天软件一样看消息：头像、引用条（点一下跳回被引用的那条并高亮）、`@昵称`、自己与机器人消息靠右；跨多天时自动插入日期分隔线；**向上滑动自动加载更早的消息** |
| 统计 | 每日消息量、活跃发言人 Top 10、24 小时时段分布，并可按日期范围（预设 + 日历自定义区间）查看；所选目标的每日总结（含**总结历史**，切换目标自动跟着换）|
| 搜索 | 在所选目标的归档里按关键词 / 发送者（QQ 号或昵称）搜索，可用顶部**日期范围**缩小范围（选「全部时间」则不限日期），结果可一键定位到具体消息 |
| 配置 | 图形化修改本插件配置（等同 WebUI 插件配置页），改完点「保存」即生效 |

使用小提示：

- 顶部只有两个控件：**目标**选择 + **日期范围**按钮（统计 / 消息预览 / 搜索 三页通用）；旧版那个单独的「日期」下拉已移除，范围按钮就是唯一的日期筛选入口。
  - 左侧预设：全部时间 / 当天 / 今天 / 昨天 / 近 7 天 / 近 30 天 / 本月 / 上月 / 自定义；
  - 选「自定义」时，在双月日历上依次点选开始与结束日期（未来日期不可选，区间内日期高亮）；选「全部时间」则不限日期。
  - 范围会同时驱动统计卡片、每日消息量、发言人 Top 10、时段分布、消息预览的日期筛选与搜索范围；总结按范围的**结束日**生成（按钮上会直接显示该日期）。
...
| aliases | []（空）| 对话别名：每项一条「别名,群号或QQ号」（如 闲聊群,123456789）；也可直接让 bot 管理 |
